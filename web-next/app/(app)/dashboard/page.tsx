          ...
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-[12px] mb-1">
                <span>98%</span><span>תשלומים בזמן</span>
              </div>
              <div className="w-full bg-[#1a365d] h-1 rounded-full overflow-hidden">
                <div className="bg-landlord-green h-full w-[98%]" />
              </div>
            </div>
            <div>
              <div className="flex justify-between text-[12px] mb-1">
                <span>4.9/5</span><span>חוות דעת משכירים</span>
              </div>
              <div className="w-full bg-[#1a365d] h-1 rounded-full overflow-hidden">
                <div className="bg-landlord-green h-full w-[92%]" />
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <header className="mb-[24px]">
        <h2 className="text-[28px] leading-[36px] font-bold text-tenant-blue mb-1">
          שלום, {user?.firstName} 👋
        </h2>
        <p className="text-[18px] leading-[26px] font-semibold text-on-surface-variant opacity-80">
          מה תרצה לעשות היום?
        </p>
      </header>
            ...
            <div className="space-y-4">
              {recommendedApts.length > 0 ? recommendedApts.map((apt) => (
                <Link
                  ...
                  className="bg-surface-container-lowest rounded-xl p-4 flex items-center soft-shadow hover:bg-surface-container-low transition-colors cursor-pointer border border-transparent hover:border-outline-variant block"
                >
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-[12px] text-on-surface-variant flex items-center">
                          ...
                          {apt.city} {apt.neighborhood ? `• ${apt.neighborhood}` : ""} • {apt.rooms} חדרים {apt.size ? `• ${apt.size} מ"ר` : ""}
                        </p>
                      </div>
                    ...
                    {activeContracts.map((c) => (
                      <tr key={c.id}>
                        <td className="px-6 py-4">
                          <p className="text-[14px] font-medium text-tenant-blue">{c.apartment?.address || "—"}</p>
                        </td>
                        <td className="px-6 py-4">
                          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[12px] font-medium bg-[#9cefdf] text-[#0b6f63]">בתוקף</span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <Link href={`/contracts/${c.id}`} className="text-[#006b5f] hover:text-[#0b6f63] transition-colors">
                ...
                {recentNotifs.map((n) => (
                  <div key={n.id} className="flex items-start">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                      n.type?.includes("payment") ? "bg-[#9cefdf] text-[#0b6f63]" : "bg-[#d6e3ff] text-[#001b3c]"
                    }`}>
                      <span className="material-symbols-outlined text-[16px]">
                        {n.type?.includes("payment") ? "check_circle" : n.type?.includes("message") ? "forum" : "notifications"}
                      </span>
                    </div>
                    <div className="mr-3">
      ...
      <Link
        href="/maintenance"
        className="fixed bottom-8 left-8 w-14 h-14 bg-landlord-green text-white rounded-full shadow-lg flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-50 group"
      >
        <span className="material-symbols-outlined text-[28px]">add</span>
        <span className="absolute right-full ml-3 bg-tenant-blue text-white px-4 py-2 rounded-lg text-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-md pointer-events-none">
          דווח על תקלה
        </span>
      </Link>
    </div>
