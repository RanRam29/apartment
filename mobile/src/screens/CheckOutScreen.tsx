...
export default function CheckOutScreen({ route, navigation }: any) {
  ...
  const { uploadCheckoutPhotos, completeCheckout, isLoading } = useContractStore();
  ...
  const handleAddPhoto = (roomId: string) => {
    // Simulate checkout photo capture
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <Text style={styles.title}>תיעוד עזיבת דירה (Check-Out)</Text>
      <Text style={styles.subtitle}>נא לצלם ולתעד את מצב החדרים לפני פינוי סופי והחזרת מפתח.</Text>
      ...
      <TouchableOpacity onPress={handleSave} style={styles.saveBtn} disabled={isLoading}>
        {isLoading ? (
          ...
          <Text style={styles.saveBtnText}>אשר והשלם צ׳ק-אאוט</Text>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
