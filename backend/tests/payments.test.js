...
describe('Payments routes', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    process.env.MESHULAM_API_KEY = 'test-key';
  });
  ...
  it('handles webhook and upgrades user on success', async () => {
    const res = await request(app)
      ...
      .send({ transactionId: 'trx-1', status: 'success', userId: 'user-1' });

    expect(res.status).toBe(200);
    expect(res.body.received).toBe(true);
    expect(User.update).toHaveBeenCalledWith({ isPremium: true }, { where: { id: 'user-1' } });
  });
